	...
	<body>
		<?php
			...
			if ($_SERVER["REQUEST_METHOD"] == "POST") 
			{
				...
				if($conn)
				{
					$query="select Package.Package_id,Package.Package_name from Package, Offers_package where Offers_package.Package_id=Package.package_id && Offers_package.Lab_id='$labid'" ;
					...
					if($result)
					{
						...
						{
							$packageid[$n]=$row['Package_id'];
							$packagename[$n]=$row['Package_name'];
							$n=$n+1;
						}
						//print_r($testid);
						//print_r($testname);
...
<br/><br/>
		<center>
		<div class="contain1" >
			<center><legend><h2><b><font style="Arial" color="black"><span>BOOK A PACKAGE</span></font></b></h2></legend></center><br/>
			<?php
				$servername="localhost";
				$username="root";
				$password="";
			...
			<form  name="form2" action="booking_package.php" method="post" >
									<select class="form-control" name="test_name" id="test_name">
										<option value="" disabled selected>Select a Package</option>
										<?php
											for($k=1;$k<$n;$k++)
											{
												echo "<option value='$packageid[$k]'>$packagename[$k]</option>" ;
											}
										?>
									</select><br/>
